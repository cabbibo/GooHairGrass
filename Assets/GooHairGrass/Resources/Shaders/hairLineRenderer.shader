Shader "Custom/hairLineRenderer" {
...
struct Vert{
  ...
  float3 triWeights;
        ...
        //float3 audio  = tex2D(_Audio, float2( v.uv)).xyz;
                ...
                float3 col = cubeCol * (normalize( eyeRefl )* .5 +.5);//v.debug;//cubeCol * (.6*audio+.7) * float3( 6 , 3 + sin(_Time.x) , 2) * .2;// float3(1,0,0);//v.debug * v.uv.x;//v.nor * .5 + .5;
                return float4( col , 1 );
